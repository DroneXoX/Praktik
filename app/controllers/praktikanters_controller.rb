class PraktikantersController < ApplicationController
  ...
  def new
    ...
    @praktikanter = current_user.praktikanter.new
  end
  ...
  def create
    ...
    @praktikanter = current_user.praktikanter.build(praktikanter_params)
    respond_to do |format|
      if @praktikanter.save
        format.html { redirect_to praktikanter_url(@praktikanter), notice: "Praktikant har skapats." }
  ...
  def update
    respond_to do |format|
      if @praktikanter.update(praktikanter_params)
        format.html { redirect_to praktikanter_url(@praktikanter), notice: "Praktikant har uppdaterats." }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to praktikanters_url, notice: "Praktikant har tagits bort." }
      format.json { head :no_content }
    end
  end

  def correct_user
    @praktikanter = current_user.praktikanter.find_by(id: params[:id])
    redirect_to praktikanter_path, notice: "Saknar behörighet" if @praktikanter.nil?
  end
  ...
  private
    ...
    def praktikanter_params
      params.require(:praktikanter).permit(:first_förnamn, :efternamn, :email, :telefon, :linkedin, :user_id)
    end
end
